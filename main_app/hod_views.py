...
def manage_staff(request):
    ...
    context = {"allStaff": allStaff, "page_title": "Manage Staff"}
    ...
    if request.method == "POST":
        if form.is_valid():
            try:
                ...
                return render(
                    request, "hod_template/edit_session_template.html", context
                )
        else:
...
def get_admin_attendance(request):
    ...
    try:
        ...
        for report in attendance_reports:
            data = {"status": str(report.status), "name": str(report.student)}
...
def admin_notify_staff(request):
    ...
    context = {"page_title": "Send Notifications To Staff", "allStaff": staff}
    return render(request, "hod_template/staff_notification.html", context)


def admin_notify_student(request):
    student = CustomUser.objects.filter(user_type=3)
    context = {"page_title": "Send Notifications To Students", "students": student}
